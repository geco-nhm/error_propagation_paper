...
for (i in start_row:end_row) {
  ...
  test_set <- paste(output_matrix$hierarchicallevel[i],
                    ifelse(grepl(output_matrix$hierarchicallevel[i], "gtype1"), ifelse(output_matrix$interpreter[i] %in% c("I20","K20","H20","G20","L20","J20","X20"),"_5","_20"),""), sep = "")
  
  #Store the thematic resolution
  output_matrix$thematicresolution[i] <- test_set
  
  #Specify sample
  train_data_loop <- train_data_loop[which(train_data_loop$ID_low %in% sample_training[[ensemble]][[which(samplesizes == output_matrix$samplesize[i])]]),]
