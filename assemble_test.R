...
#Reclassify boreal heath above 100 m
...
#Extract elevation data for vector layer
vector_layer$elevation <- data.frame(elevation = extract(raster_list[[3]]$dem_med, cell_number))

#Identify observations elevated above 100 m
filter_1 <- which(vector_layer$elevation >= 100) 

#Identify observations of boreal heath
filter_2 <- which(vector_layer$htype1 == "T31")

#Identify observations of boreal heath that are elevated above 100 m
vector_reclassify <- filter_1[filter_1 %in% filter_2]

#Reclassify boreal heath 
vector_layer$gtype1[vector_reclassify] <- "T2-C-1"
vector_layer$htype1[vector_reclassify] <- "T2"
vector_layer$htypegr1[vector_reclassify] <- "T"
...
#Save vector layer
st_write(vector_layer, dsn = "data/processed/test/", layer = "test_points.shp", driver = "ESRI Shapefile", append = F)