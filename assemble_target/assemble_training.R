...
calculate.mode <- function(vector_object, map_scale) {
  ...
  #Ordering the vector based on experience (have to change this when the column is renamed)
  if (map_scale == "fine") {
...
fine <- c("A5","F5","D5","C5","B5","E5")

#Specify interpreters with 1:20000 maps
coarse <- c("I20","K20","H20","G20","L20","J20")
...
for (i in 1:length(target_matrices)) {
  for (j in 1:length(hierarchical_levels)) {
    ...
    #Rename the column
    colnames(target_matrices[[i]])[ncol(target_matrices[[i]])] <- paste(hierarchical_levels[j],"_X5", sep = "")
    ...
    #Rename the column
    colnames(target_matrices[[i]])[ncol(target_matrices[[i]])] <- paste(hierarchical_levels[j],"_X20", sep = "")
  }
}
